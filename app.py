"""DermSAM Gradio demo — automatic skin lesion segmentation.

HuggingFace Spaces entry point and local launcher.
Internal flow: image → localizer → auto bbox → MedSAM → mask overlay returned.

Usage:
    python app.py --localizer-ckpt checkpoints/best_localizer.pth --medsam-ckpt checkpoints/medsam_vit_b.pth
"""
...
def build_interface() -> gr.Blocks:
    ...
    """
    examples_dir = Path(__file__).parent / "app" / "examples"
    example_images = sorted(examples_dir.glob("*.jpg")) + sorted(examples_dir.glob("*.png"))
    examples = [[str(p)] for p in example_images] if example_images else None
...
if __name__ == "__main__":
    ...
    # HuggingFace Spaces runs app.py as a module — load models with default paths
    set_seed(42)
    load_models(Path("checkpoints/best_localizer.pth"), Path("checkpoints/medsam_vit_b.pth"), device="cpu")
